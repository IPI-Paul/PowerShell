...
function Get-Headers {
    ...
    try {
        ...
        # Build a single "A" row
        $aRow = (@('A') * $colCount) -join ','

        # Write 8 A-filled rows
        1..20 | ForEach-Object { $aRow | Add-Content $dest }
...
function Get-SpecialColumns {
    ...
    $ws1.Range("A2").CopyFromRecordset($rs)
    $rowCount = $ws1.UsedRange.Rows.Count() + 1
    $ws1.Rows("2:21").Delete()
    ...
    for ($i = 1; $i -le $colCount; $i++) {
        ...
        $formula    = @"
=LET(
  src, OFFSET(Sheet1!$($colLetter)1,,, COUNTA(Sheet1!$($colLetter):$colLetter), 1),
  res, INDEX(FILTER(src, UPPER(src) > ""), SEQUENCE(50,,2)),
  TEXTJOIN("|", TRUE, IFERROR(INDEX(SORT(CHOOSE({1, 2}, LEN(res),res), 1, -1),, 2), ""))
)
"@
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="2">
        ...
        <ListView Name="CsvList" Grid.Row="0" Margin="0, 0, 0, 2">
            <ListView.View>
                <GridView>
                    <GridViewColumn Header="#" Width="50" DisplayMemberBinding="{Binding Number}" />
                    <GridViewColumn Header="CSV Path" Width="500" DisplayMemberBinding="{Binding Path}" />
                </GridView>
            </ListView.View>
        </ListView>

        <StackPanel  Grid.Row="1" Orientation="Horizontal" HorizontalAlignment="Right">
            <Button Name="OkButton" Width="80" Margin="2">OK</Button>
            <Button Name="CancelButton" Width="80" Margin="2">Cancel</Button>
        </StackPanel>
    </Grid>
</Window>
"@
...
function Update-FormatAndData {
    ...
    # Load CSV via ADODB and force special columns as text
    $conn       = New-Object -ComObject ADODB.Connection
    $rs         = New-Object -ComObject ADODB.Recordset
    $connStr    = "Provider=Microsoft.ACE.OLEDB.16.0;Data Source=$Global:Folder;Extended Properties='Text;HDR=Yes;FMT=Delimited'"
    $conn.Open($connStr)
    ...
    # Release COM objects
    if ($null -ne $rs) { [System.Runtime.InteropServices.Marshal]::ReleaseComObject($rs) | Out-Null }
    if ($null -ne $conn) { [System.Runtime.InteropServices.Marshal]::ReleaseComObject($conn) | Out-Null }
